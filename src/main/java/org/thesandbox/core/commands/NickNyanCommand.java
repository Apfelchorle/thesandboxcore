...
public class NickNyanCommand implements ISubCommand
{
    private final JavaPlugin plugin;
    private final Essentials essentials;
    private final Random rng = new Random();

    // Only actual color codes (no formatting like BOLD/UNDERLINE/MAGIC/RESET)
    private static final ChatColor[] COLOR_POOL = new ChatColor[] {
    ...
    public NickNyanCommand(JavaPlugin plugin)
    {
        this.plugin = plugin;
        Essentials ess = null;
        try {
    ...
    public boolean execute(CommandSender sender, Command command, String label, String[] args)
    {
        ...
        // /nicknyan off -> clear nickname
        if (args.length >= 1 && args[0].equalsIgnoreCase("off"))
        {
            user.setNickname(null);
            sender.sendMessage(CommandMessages.command(ChatColor.YELLOW + "Nickname cleared."));
            return true;
        }

        // Determine base text:
        // - If arg provided, use it.
        // - Else if player has a nickname, nyanify that nickname's VISIBLE text.
        ...
        // Build randomized-color nickname per codepoint
        String nyanNick = nyanifyVisible(visible);
        String finalNick = nyanNick + ChatColor.WHITE; // reset to avoid bleed

        user.setNickname(finalNick);
        sender.sendMessage(CommandMessages.command(ChatColor.YELLOW + "Your nickname is now: " + finalNick));
        return true;
    }
    ...
    /** Random color per codepoint of the provided VISIBLE (no colors) text. */
    private String nyanifyVisible(String text)
    {
        StringBuilder out = new StringBuilder();
        int[] cps = text.codePoints().toArray();
        for (int cp : cps)
        {
            ChatColor cc = COLOR_POOL[rng.nextInt(COLOR_POOL.length)];
            out.append(cc).append(new String(Character.toChars(cp)));
        }
        return out.toString();
    }
}