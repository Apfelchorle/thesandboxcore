...
/**
 * /tagrainbow [text...]
 * - If text is provided: rainbow-ify that text and set as your tag (session-only).
 * - If no text: rainbow-ify your existing tag (if any).
 * - Use /tag save to persist the current tag afterward.
 */
public class TagRainbowCommand implements ISubCommand
{
    ...
    private final TagService tagService; // saving handled via /tag save, not here

    public TagRainbowCommand(JavaPlugin plugin, TagService tagService)
    {
        this.plugin = plugin;
        ...
        Bukkit.getScheduler().runTaskAsynchronously(plugin, () -> {
            try
            {
                ...
                else
                {
                    ...
                    {
                        p.sendMessage(CommandMessages.usage(ChatColor.RED + "Usage: /tagrainbow <tag>"));
                        return;
                    }
                ...
                String rainbow = rainbowify(baseVisible);
                String finalPrefix = ensureSingleSpaceAfterTag(rainbow); // trims whitespace + dangling codes, adds one space + reset

                // Apply session-only: remove existing prefix nodes at our priority, then add
                ...
                p.sendMessage(CommandMessages.command(ChatColor.GREEN + "Rainbow tag applied (session-only)."));
    ...
    {
        // No static suggestions; user types their text.
        return List.of();
    }
    ...
    /** Classic rainbow across visible characters. */
    private static String rainbowify(String text)
    {
        // ROYGBIV using closest Bukkit colors
        ChatColor[] colors = new ChatColor[]{
            ChatColor.RED, ChatColor.GOLD, ChatColor.YELLOW,
            ChatColor.GREEN, ChatColor.BLUE, ChatColor.DARK_PURPLE, ChatColor.LIGHT_PURPLE
        };
    ...
    // ===== Blacklist (same behavior as TagCommand) =====
    ...
    private String firstBlacklistedTerm(String rawInputWithPossibleCodes)
    {
        ...
        for (String term : getTagBlacklist())
        {
            ...
            if (t.regionMatches(true, 0, "regex:", 0, 6))
            {
                ...
                try {
                    if (plain.matches("(?s).*" + pattern + ".*")) return term;
                } catch (Exception ignored) {}
            }
            else
            {
                if (plain.contains(t.toLowerCase(Locale.ROOT))) return term;
            }
        }
        return null;
    }
